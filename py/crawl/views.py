...
from models import MatchResult, STATUS_ICON
...
def save_json(results: list[MatchResult], filename: str = "eos_results.json") -> None:
    output = [
        {
            "buyzoxs_slug": r.slug,
...
def save_ods(results: list[MatchResult], filename: str = "eos_results.ods") -> None:
    ...
    # FIXED: Correct odfpy syntax for TableHeaderRows
    ...
    for r in sorted_results:
        ...
        variants_to_export = r.variants if r.variants else [{"zustand": "Unknown", "price": "See website"}]
    ...
    db_ranges.addElement(DatabaseRange(name="__AutoFilter__", targetrangeaddress=f"eos-buyzoxs.A1:{col_letter}{total_rows}", displayfilterbuttons="true"))
